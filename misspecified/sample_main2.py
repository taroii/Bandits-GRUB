"""
Sample main 2 for misspecified graph setting.
Sweeps over arm counts comparing Misspecified TS-Explore vs base algorithms.
"""
...
if __name__ == "__main__":
    ...
    clusters = 10
    results = {name: [] for name in ['Cyc', 'GB', 'GB_2', 'GB_sum', 'Base', 'TS', 'TS_mis']}
    ...
    for total_arms in arm_counts:
        ...
        results['TS_mis'].append(run_algo(TS_mis, printer="TS-Explore-Mis", nodes=nodes, t=TS_mis.t))

    labels = {'Cyc': 'Cyclic', 'GB': 'JVM-O', 'GB_2': 'MVM', 'GB_sum': 'JVM-N', 'Base': 'No Graph UCB', 'TS': 'TS-Explore', 'TS_mis': 'TS-Explore-Mis'}
    for name, times in results.items():
        plt.plot(arm_counts, times, marker='o', label=labels[name])

    plt.xlabel("Number of arms")
    plt.ylabel("Time steps (arm pulls)")
    plt.title("Sample Complexity vs Number of Arms (Misspecified)")
    plt.legend()
    plt.grid()
