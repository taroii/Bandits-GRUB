"""
Sample main for misspecified graph setting.
Single-run comparison of Misspecified TS-Explore vs base algorithms.
"""
...
from misspecified.misspecified_algo import MisspecifiedThompsonSampling
...
if __name__ == "__main__":
    ...
    # eps_mis defaults to half the true smoothness
    TS_mis = MisspecifiedThompsonSampling(Degree, Adj, node_means, eta=eta, delta=0.0001, q=0.01)
    ...
    Time_tracker_TS_mis, _ = run_algo(TS_mis, printer="TS-Explore-Mis", cluster_size=node_per_cluster, nodes=nodes, t=TS_mis.t)
    ...
    plt.plot(Time_tracker_TS_mis, len(node_means)*np.ones(len(Time_tracker_TS_mis)) - range(len(Time_tracker_TS_mis)), color='brown', marker='x', markersize=4, label='TS-Explore-Mis', linewidth=2.0, linestyle='-.')
    ...
    plt.title("No. of remaining arms vs time steps (Misspecified)")
