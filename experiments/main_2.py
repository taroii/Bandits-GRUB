...
LEGACY_ALGOS = ['TS-Explore', 'Basic TS', 'GRUB']
...
def make_factory(name, D, A, mu, delta, q, rho):
    ...
    if name == 'KL-LUCB':
        return KLLUCBFactory(mu, delta=delta)
    ...
    if os.path.exists(out_npz) and not args.fresh:
        try:
            ...
            if (prev_Ks == Ks and prev_seeds == seeds
                ...
                prev_algos = (list(z['algos'].astype(str).tolist())
                              if 'algos' in z.files else list(LEGACY_ALGOS))
                prev_done = z['done'].astype(bool)
                # Translate prev_done from prev_algos columns to current ALGOS
                # columns; new algos start unfinished so their column is False.
                done_new = np.zeros((len(Ks), len(ALGOS)), dtype=bool)
                for ai, name in enumerate(ALGOS):
                    if name in prev_algos:
                        ai_old = prev_algos.index(name)
                        if ai_old < prev_done.shape[1]:
                            done_new[:, ai] = prev_done[:, ai_old]
                done = done_new
                for a in ALGOS:
