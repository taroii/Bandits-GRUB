"""
Sample main 2 for graph feedback setting (TS-Explore-GF).
Sweeps over arm counts comparing TS-Explore-GF vs base algorithms.
"""
...
if __name__ == "__main__":
    ...
    clusters = 10
    results = {name: [] for name in ['Cyc', 'GB', 'GB_2', 'GB_sum', 'Base', 'TS', 'TS_GF']}
    ...
    for total_arms in arm_counts:
        ...
        results['TS_GF'].append(run_algo(TS_GF, printer="TS-Explore-GF", nodes=nodes, t=TS_GF.t))

    labels = {'Cyc': 'Cyclic', 'GB': 'JVM-O', 'GB_2': 'MVM', 'GB_sum': 'JVM-N', 'Base': 'No Graph UCB', 'TS': 'TS-Explore', 'TS_GF': 'TS-Explore-GF'}
    for name, times in results.items():
        plt.plot(arm_counts, times, marker='o', label=labels[name])

    plt.xlabel("Number of arms")
    plt.ylabel("Time steps (arm pulls)")
    plt.title("Sample Complexity vs Number of Arms (Graph Feedback)")
    plt.legend()
    plt.grid()
