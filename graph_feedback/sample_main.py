"""
Sample main for graph feedback setting (TS-Explore-GF).
Single-run comparison of TS-Explore-GF vs base TS-Explore and UCB methods.
"""
...
from graph_feedback.graph_feedback_algo import GraphFeedbackTS
...
if __name__ == "__main__":
    ...
    TS_GF = GraphFeedbackTS(Degree, Adj, node_means, delta=0.0001, q=0.01)
    ...
    Time_tracker_TS, _ = run_algo(TS, printer="Thompson Sampling", cluster_size=node_per_cluster, nodes=nodes, t=TS.t)
    Time_tracker_TS_GF, _ = run_algo(TS_GF, printer="TS-Explore-GF", cluster_size=node_per_cluster, nodes=nodes, t=TS_GF.t)
    ...
    plt.plot(Time_tracker_TS_GF, len(node_means)*np.ones(len(Time_tracker_TS_GF)) - range(len(Time_tracker_TS_GF)), color='purple', marker='D', markersize=3, label='TS-Explore-GF', linewidth=2.0, linestyle='-.')
    ...
    plt.title("No. of remaining arms vs time steps (Graph Feedback)")
